...
const brandMerchandisingSchema = z.object({
  id: uuidSchema,
  /** Blank means "let real sales decide"; a number pins the brand there. */
  merchandisingRank: z
    .string()
    .trim()
    .transform((v) => (v === '' ? null : Number(v)))
    .refine((v) => v === null || (Number.isInteger(v) && v >= 1 && v <= 99), {
      message: 'Enter a position between 1 and 99, or leave it blank.',
    }),
  featured: z.boolean(),
});
...
export async function saveBrandMerchandising(
  ...
  const parsed = brandMerchandisingSchema.safeParse({
    id: formData.get('id'),
    merchandisingRank: String(formData.get('merchandisingRank') ?? ''),
    featured: formData.get('featured') === 'on',
  });
  if (!parsed.success) {
    return actionError('Check the brand fields.', toFieldErrors(parsed.error));
  }
  const input = parsed.data;
  ...
  if (!updated[0]) return actionError('Brand not found.');
  ...
  await recordAudit({
    ...
    entityId: input.id,
    changes: {
      merchandisingRank: { from: null, to: input.merchandisingRank },
      featured: { from: null, to: input.featured },
    },
  });
